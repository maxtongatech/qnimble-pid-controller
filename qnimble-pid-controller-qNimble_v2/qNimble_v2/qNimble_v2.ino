...
SquareWave_Manager sqwave_manager;
...
void setup() {
    ...
    command_handler.init(&adc_manager, &dac_manager, &nvm_manager, 
                        &servo_manager, &Serial, &sqwave_manager);
    Serial.println("✓ Command handler ready");

    Serial.println("Initializing Square Wave Manager...");
    sqwave_manager.init();
    Serial.println("✓ Square Wave Manager ready");
...
void applyConfigToHardware() {
    ...
    for (int i = 1; i <= NUM_ADC_CHANNELS; i++) {
        SquareWave_Generator* sqw = sqwave_manager.getGenerator(i);
        if (sqw) {
            float period = nvm_manager.getSquareWavePeriod(i);
            float duty = nvm_manager.getSquareWaveDuty(i);
            float high = nvm_manager.getSquareWaveHigh(i);
            float low = nvm_manager.getSquareWaveLow(i);

            sqw->begin(period, duty);
            sqw->setHighSetpoint(high);
            sqw->setLowSetpoint(low);

            if (nvm_manager.getSquareWaveEnabled(i)) {
                sqwave_manager.enableChannel(i);
            }
        }
    }
...
void loop() {
    ...
    // Update square waves
    sqwave_manager.update();

    // If enabled: apply square wave's setpoints to servos
    for (int i = 1; i <= NUM_ADC_CHANNELS; i++) {
        if (sqwave_manager.isChannelEnabled(i)) {
            PID_Controller* pid = servo_manager.getController(i);
            if (pid) {
                pid->setSetpoint(sqwave_manager.getCurrentSetpoint(i));
            }
        }
    }
    ...
    if (Serial.available()) {
        ...
        if (cmd.length() == 0) {
            ...
            switch (single) {
                ...
                case 'z':  // Square wave status
                case 'Z':
                    sqwave_manager.printStatus(Serial);
                    break;
...
void printHelp() {
    ...
    Serial.println("  'z' - Print square wave status");
