...
class SerialHandler(QObject):
    ...
    def get_p_gain(self, channel):
        ...
        response = self.send_command(f"prop{channel}")
        if not response:
            return None
        try:
            # Extract number from response
            # Handle formats like "0.001", "P1 = 0.001", "Channel 1 P gain: 0.001"
            numbers = re.findall(r'[-+]?\d*\.?\d+', response)
            if numbers:
    ...
    def get_i_gain(self, channel):
        ...
        response = self.send_command(f"int{channel}")
        if not response:
            return None
        try:
            # Extract number from response
    ...
    def get_d_gain(self, channel):
        ...
        response = self.send_command(f"deriv{channel}")
        if not response:
            return None
        try:
            # Extract number from response
    ...
    def get_setpoint(self, channel):
        ...
        response = self.send_command(f"setpoint{channel}")
        if not response:
            return None
        try:
            # Extract number from response
